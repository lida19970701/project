...
router.post('/manage', function(req, res, next) {
	...
	shopModel.findAll({
		...
		goodsModel.findAll({
			...
			// let goods = {}
			// goods.goodsid = rss[0].id
			// goods.goodsname = rss[0].goodsname;
			// goods.goodsprice = rss[0].goodsprice;
			// goods.goodsnum = rss[0].goodsnum;
			// goods.goodspath = rss[0].goodspath;
			// goods.shopname = rs[0].shopname;
...
router.post('/goods', function(req, res, next) {
	...
	form.parse(req, function(err, fields, files) {
		...
		let sql = 'insert into goods (uid,shopid,goodsname,goodstype,goodsprice,goodsnum,goodspath) value(?,?,?,?,?,?,?)'
		sequelize.query(sql, {
			replacements: [goods.uid, goods.shopid, goods.goodsname, goods.goodstype, goods.goodsprice, goods.goodsnum, goods.goodspath]
		}).then(function(rs) {
			let sq = 'select shopname from shoptables where id=?'
			sequelize.query(sq, {
				replacements: [goods.shopid]
			}).then(function(rss) {
				if (rss) {
					let shop = {}
					shop.goodsid = rs[0];
					shop.goodsname = goods.goodsname;
					shop.goodsnum = goods.goodsnum;
					shop.goodspath = goods.goodspath;
					shop.goodsprice = goods.goodsprice;
					shop.shopname = rss[0][0].shopname
					elastic.addDocument(shop).then(function(result) {
